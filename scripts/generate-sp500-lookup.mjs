...
/** Parse first two CSV fields (Symbol, Security); handles quoted commas. */
function parseSymbolSecurity(line) {
  ...
  const symbol = readField();
  const security = readField();
...
function main() {
  ...
  for (let li = 1; li < lines.length; li++) {
    const { symbol, security } = parseSymbolSecurity(lines[li]);
