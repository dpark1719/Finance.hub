...
function parseWikiComponents(wikitext) {
  ...
  for (const line of chunk.split("\n")) {
    const t = line.trim();
    const m = /^\|\s*(.+?)\s*\|\|\s*(\d{6})\s*\|\|\s*(.+?)\s*\|?\s*$/.exec(t);
    if (!m) continue;
    const name = wikiDisplayName(m[1]);
    const code = m[2];
    const sector = wikiDisplayName(m[3]);
    if (name && /^\d{6}$/.test(code) && sector) {
      out.push({ name, code, sector });
    }
  }
  return out;
}
...
function main() {
  ...
  const kospi200ByCode = new Map();
  for (const { name, code, sector } of wikiRows) {
    if (!/^\d{6}$/.test(code)) continue;
    kospi200ByCode.set(code, {
      sym: codeToSym(code),
      name,
      sector: sector.trim() || "Unknown",
    });
  }
  const kospi200Sorted = [...kospi200ByCode.entries()].sort((a, b) =>
    a[0].localeCompare(b[0]),
  );
  const kospi200TickerEntries = kospi200Sorted
    .map(([, v]) => `  ${JSON.stringify(v.sym)},`)
    .join("\n");
  const kospi200GicsEntries = kospi200Sorted
    .map(
      ([, v]) =>
        `  ${JSON.stringify(v.sym)}: { sector: ${JSON.stringify(v.sector)}, industry: ${JSON.stringify(v.sector)} },`,
    )
    .join("\n");
  const kospi200NameEntries = kospi200Sorted
    .map(([, v]) => `  ${JSON.stringify(v.sym)}: ${JSON.stringify(v.name)},`)
    .join("\n");
...
/** KOSPI 200 index members (Wikipedia components table) — ######.KS symbols */
export const KOSPI_200_TICKERS: readonly string[] = [
${kospi200TickerEntries}
];

/** English display names from Wikipedia KOSPI 200 table */
export const KOSPI_200_NAMES: Record<string, string> = {
${kospi200NameEntries}
};

/** GICS-style sector from Wikipedia (used as sector + industry in heatmap / panel) */
export const KOSPI_200_GICS: Record<string, { sector: string; industry: string }> = {
${kospi200GicsEntries}
};

const KOSPI_CODE_SET = new Set(KOSPI_CODES);
const KOSPI_200_SET = new Set(KOSPI_200_TICKERS);
...
`;
  ...
  console.error(
    `Wrote ${outFile} (${sortedKeys.length} lookup keys, ${codes.length} codes, ${kospi200Sorted.length} KOSPI 200).`,
  );
}
